...
export default function PageForm({
  ...
  const [formData, setFormData] = useState({
    emails: data?.recipients
      ? JSON.parse(data?.recipients).join(', ')
      : 'custom',
    subject: data?.title,
    body: data?.body,
  ...
  useEffect(() => {
    if (data?.recipients) {
      setSelectedUsers(JSON.parse(data?.recipients?.replace(/,\s*$/, '')));
    }

    setFormData({ ...data, subject: data?.title, status: data?.status });
  }, [data]);
    ...
    >
      ...
      <Box sx={{ my: 3 }}>
        ...
        <TextField
          ...
          required
        >
          <MenuItem value="all" onClick={() => setEmailType('all')}>
            All Users
          </MenuItem>
          <MenuItem value="active" onClick={() => setEmailType('active')}>
            Active Users
          </MenuItem>
          <MenuItem value="inactive" onClick={() => setEmailType('inactive')}>
            Inactive Users
          </MenuItem>
          <MenuItem value="disabled" onClick={() => setEmailType('disabled')}>
            Disabled Users
          </MenuItem>
